...
namespace DistributedPrimeCalculatorApp
{
    public class MainWorker : Worker
    {
        public override void Main()
        {
            ...
            var workers = new List<WorkerAddress>();
            var remoteNodeIds = Node.GetRemoteNodeIds();
            foreach (ushort nodeId in remoteNodeIds)
            {
                ...
                Log.Info("Spawning {0} workers at node {1}, address {2}", processorCount, nodeId,
                    Node.GetIpEndPoint(nodeId).Address);
                for (int i = 0; i < processorCount; i++)
                {
                    var worker = Spawn<PrimeWorker>(nodeId); //here we specify the exact node id
                    workers.Add(worker);
