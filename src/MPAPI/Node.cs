...
namespace MPAPI
{
    public sealed class Node : INode, IWorkerNode, IRegisteredNode, IDisposable
    {
        ...
        private readonly ManualResetEvent _sendBufferWaitHandle = new ManualResetEvent(false);
        private ServiceHost _host;
        private IRegistrationServer _registrationServerProxy;
        private readonly Dictionary<ushort, Worker> _localWorkers = new Dictionary<ushort, Worker>();
        private readonly Dictionary<ushort, INode> _remoteNodes = new Dictionary<ushort, INode>();
        private ushort _id;
        private readonly object _idLock = new object();
        private readonly LinkedList<Message> _sendBuffer = new LinkedList<Message>();
        private bool _sendBufferThreadTerminate = false;
        private readonly object _sendBufferThreadTerminateLock = new object();
        private readonly Dictionary<Type, byte> _parameterTypes;
        ...
        public Node()
        {
            ...
            {
                {typeof(bool), ContentTypes.Primitive},
                {typeof(byte), ContentTypes.Primitive},
                {typeof(sbyte), ContentTypes.Primitive},
                {typeof(char), ContentTypes.Primitive},
                {typeof(decimal), ContentTypes.Primitive},
                {typeof(double), ContentTypes.Primitive},
                {typeof(float), ContentTypes.Primitive},
                {typeof(int), ContentTypes.Primitive},
                {typeof(uint), ContentTypes.Primitive},
                {typeof(long), ContentTypes.Primitive},
                {typeof(ulong), ContentTypes.Primitive},
                {typeof(short), ContentTypes.Primitive},
                {typeof(ushort), ContentTypes.Primitive},
                {typeof(string), ContentTypes.Primitive}
            };

            //start the thread that processes outgoing messages
            var sendBufferThread = new Thread(SendBufferThreadProc)
            {
                IsBackground = true
            };
        ...
        public void OpenDistributed(string registrationServerNameOrAddress, int registrationServerPort,
            int listenerPort)
        {
        ...
        /// <param name="listenerPort">The port that this node accepts connections on.</param>
        /// <returns>The worker instance if successfull, otherwise null</returns>
        public TRootWorker OpenDistributed<TRootWorker>(string registrationServerNameOrAddress,
            int registrationServerPort, int listenerPort) where TRootWorker : Worker
        {
            OpenAndConnectToRegistrationServer(registrationServerNameOrAddress, registrationServerPort, listenerPort);
        ...
        public TRootWorker OpenLocal<TRootWorker>() where TRootWorker : Worker
        {
            ...
            if (!Spawn(typeof(TRootWorker), out ushort workerId))
        ...
        /// <param name="registrationServerPort">The port number to the registration server.</param>
        /// <param name="listenerPort">The port that this node accepts connections on.</param>
        private void OpenAndConnectToRegistrationServer(string registrationServerNameOrAddress,
            int registrationServerPort, int listenerPort)
        {
            if (_host == null)
            {
                try
                {
                   ...
                   var remoteNodeEndPoints = _registrationServerProxy.GetAllNodeEndPoints();
                    _remoteNodes.Clear();
                    foreach (var endpoint in remoteNodeEndPoints)
                    {
            ...
            if (msg.ReceiverAddress.NodeId == _id) //it is a local worker - dispatch it immediately
            {
                lock (_localWorkers)
                {
                    if (_localWorkers.ContainsKey(msg.ReceiverAddress.WorkerId))
                        _localWorkers[msg.ReceiverAddress.WorkerId].PutMessage(msg.MessageLevel,
                            msg.ReceiverAddress.NodeId, msg.ReceiverAddress.WorkerId, msg.SenderAddress.NodeId,
                            msg.SenderAddress.WorkerId, msg.MessageType, CloneContent(msg.Content));
                }
            }
            else
            {
        ...
        internal bool Spawn(Type workerType, ushort nodeId, out ushort workerId)
        {
            ...
            lock (_remoteNodes)
            {
                if (_remoteNodes.ContainsKey(nodeId))
                {
                    ...
                    {
                        Log.Error("Node.Spawn({0}, {1}, out workerId) : Remote node {1} appears to be offline",
                            workerType.FullName, nodeId);
                    ...
                    if (WorkerAddress.IsBroadcastAddress(msg.ReceiverAddress))
                    {
                        ...
                        lock (_localWorkers)
                        {
                            foreach (Worker worker in _localWorkers.Values)
                                if (msg.SenderAddress.NodeId != _id || worker.Id != msg.SenderAddress.WorkerId)
                                    worker.PutMessage(msg.MessageLevel, msg.ReceiverAddress.NodeId,
                                        msg.ReceiverAddress.WorkerId, msg.SenderAddress.NodeId,
                                        msg.SenderAddress.WorkerId, msg.MessageType,
                                        CloneContent(msg.Content)); //clone to prevent shared state
                        }
                        ...
                        lock (_remoteNodes)
                        {
                            foreach (INode remoteNode in _remoteNodes.Values)
                                remoteNode.PutMessage(msg.MessageLevel, msg.ReceiverAddress.NodeId,
                                    msg.ReceiverAddress.WorkerId, msg.SenderAddress.NodeId, msg.SenderAddress.WorkerId,
                    ...
                    else //not a broadcast, not a local receiver, then find the remote node to send it to
                    {
                        lock (_remoteNodes)
                        {
                            ...
                            if (_remoteNodes.ContainsKey(nodeId))
                            {
                                try
                                {
                                    _remoteNodes[nodeId].PutMessage(msg.MessageLevel, msg.ReceiverAddress.NodeId,
                                        msg.ReceiverAddress.WorkerId, msg.SenderAddress.NodeId,
                                        msg.SenderAddress.WorkerId, msg.MessageType, msg.Content);
                                }
                                catch (Exception)
                                {
                                    Log.Error("Node.SendBufferThreadProc : Remote node {0} appears to be offline",
                                        nodeId);
                                    _remoteNodes.Remove(nodeId);
                                    _registrationServerProxy.UnregisterNode(nodeId);
                                }
        ...
        private object CloneContent(object content)
        {
            ...
            byte contentTypeByte = _parameterTypes.ContainsKey(contentType)
                ? _parameterTypes[contentType]
            ...
            else
            {
                lock (_remoteNodes)
                {
                    ...
                    if (_remoteNodes.ContainsKey(monitoreeNodeId))
                    {
                        ...
                        {
                            Log.Error("Node._Monitor({0}, {1}) : Remote node {2} appears to be offline", monitor,
                                monitoree, monitoree.NodeId);
        ...
        public bool Spawn(Type workerType, out ushort workerId)
        {
            ...
            if (!(oWorker is Worker))
                throw new TypeLoadException($"The type '{workerType.FullName}' does not inherit from MPAPI.Worker");

            var worker = oWorker as Worker;
            lock (_localWorkers)
            {
                //find the next id
                workerId = 0;
                while (_localWorkers.ContainsKey(workerId))
                {
                    ...
                    {
                        Log.Error("Unable to spawn more workers. There are already {0} workers running",
                            _localWorkers.Count);
                        return false;
                    }
                ...
                Thread workerThread = new Thread(worker._Main)
                {
                    IsBackground = true
                }; //we use a ThreadStart since the _Start method has the wrong signature for a thread method
                workerThread.Start();
        ...
        public IPEndPoint GetIpEndPoint() => _host.EndPoint;
