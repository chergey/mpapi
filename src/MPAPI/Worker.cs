...
namespace MPAPI
{
    public abstract class Worker : IWorker
    {
        ...
        #region Static
        private static readonly Dictionary<int, Worker> ThreadIdToWorkerMap = new Dictionary<int, Worker>();

        private static void RegisterWorker(Worker worker)
        {
            lock (ThreadIdToWorkerMap)
            {
                ThreadIdToWorkerMap.Add(CurrentThread.ManagedThreadId, worker);
            }
        }
        ...
        private static void UnregisterWorker()
        {
            lock (ThreadIdToWorkerMap)
            {
                ThreadIdToWorkerMap.Remove(CurrentThread.ManagedThreadId);
            }
        }
        ...
        public static IWorker Current
        {
            get
            {
                lock (ThreadIdToWorkerMap)
                {
                    if (ThreadIdToWorkerMap.ContainsKey(CurrentThread.ManagedThreadId))
                        return ThreadIdToWorkerMap[CurrentThread.ManagedThreadId];
                    return null;
                }
            }
        }
        ...
        private Node _node;
        private readonly LinkedList<Message> _messageQueue = new LinkedList<Message>();
        private readonly ManualResetEvent _messageQueueWaitHandle = new ManualResetEvent(false);
        private WorkerAddress _monitoringWorker;
        private readonly List<int> _messageFilters = new List<int>();

        #region Properties
        public IWorkerNode Node => _node;
        ...
        internal void _Main()
        {
            ...
            try
            {
                ...
                {
                    Message msg = new Message(MessageLevel.System, _monitoringWorker, new WorkerAddress(_node.GetId(), Id), SystemMessages.WorkerTerminated, null);
            ...
            {
                var workerAddress = new WorkerAddress(_node.GetId(), Id);
        ...
        public void Monitor(WorkerAddress monitoree) => _node._Monitor(new WorkerAddress(_node.GetId(), Id), monitoree);
        ...
        public WorkerAddress Spawn<TWorkerType>() where TWorkerType : Worker
        {
            ...
            if (_node.Spawn(typeof(TWorkerType), out ushort workerId))
        ...
        public void Send(WorkerAddress receiverAddress, int messageType, object content)
        {
            ...
            Message msg = new Message(MessageLevel.User, receiverAddress, new WorkerAddress(_node.GetId(), Id), messageType, content);
            _node.QueueMessage(msg);
        }
        ...
        {
            Message msg = new Message(MessageLevel.User, new WorkerAddress(ushort.MaxValue, ushort.MaxValue), new WorkerAddress(_node.GetId(), Id), messageType, content);
            _node.QueueMessage(msg);
        }
        ...
        public Message Receive()
        {
            ...
            do
            {
                ...
                lock (_messageQueue)
                {
                    var msgNode = _messageQueue.First;
                    if (msgNode != null)
                    {
                        msg = msgNode.Value;
